...
- 数字状态到达 `8/9` 时需要变色提醒接近填满，达到 `9/9` 时显示填满状态。
...
- 右侧数字状态增加 `8/9` 接近填满状态变色，和 `9/9` 已填满状态区分显示。
- 修正首次操作指导弹窗的展示逻辑，弹窗改为页面默认显示，移除跨页面本地关闭记录，避免部署到 GitHub Pages 后受旧缓存或旧关闭状态影响；关闭后仅本次页面不再显示。
