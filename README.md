...
- 数字输入：提供 `1` 到 `9` 的触屏数字按钮，以及清除、确认按钮，方便手机和平板用户在选中格子后直接点按操作。
...
- 操作说明应覆盖手机触屏玩法：点选格子后使用数字输入区填数、清除和确认。
...
- 鼠标右键长按：确认当前格子的临时数字。
- 鼠标单击某个格子：高亮显示所有相同数字。
- 手机或平板触屏操作：先点选格子，再使用数字输入区的 `1` 到 `9`、清除和确认按钮完成操作。
...
- 新增触屏数字输入区：手机用户点选格子后，可用 `1` 到 `9` 按钮填数，并使用清除、确认按钮完成操作；首次操作提示和右侧操作说明同步补充手机玩法。
